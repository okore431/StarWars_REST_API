...
def create_user():
    ...
    earth = User(username=request_body_user ["username"], email= request_body_user ["email"], password=request_body_user ["password"])
...
def update_user(user_id):
    ...
    db.session.commit()
    serialized_user = user1.serialize()
...
def new_charecter_post():

    request_body_char = request.get_json()
    char = Characters(character_name=request_body_char["character_name"], height=request_body_char["height"], weight=request_body_char["weight"], eye_color=request_body_char["eye_color"])
    db.session.add(char)
    db.session.commit()
...
def planet_post():
    ...
    planet = Planets(planet_name=request_body_planet["planet_name"], population=request_body_planet["population"], rotation_period=request_body_planet["rotation_period"], climate=request_body_planet["climate"], orbital_period=request_body_planet["orbital_period"], gravity=request_body_planet["gravity"], terrain=request_body_planet["terrain"])
